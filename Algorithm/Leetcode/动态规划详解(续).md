...
```
一种简单直观但效率不是最高的方法是遍历从1到n的所有数字，计算每个数字中1出现的次数，然而这种方法在n非常大时会导致时间复杂度过高。

一个更有效的方法是使用数学思想来解决这个问题，即对于每一个位（个位、十位、百位...），分别计算出1出现的次数。对于第 d 位（d 表示位的级别，1 代表个位，10 代表十位...），计算方法如下：

使用n除以10^(d+1)得到d位左边的数字。
使用n模10^d得到d位右边的数字。
根据d位当前的数字，分几种情况来计算d位1出现的次数：
如果d位的数字是0，则1出现的次数为左边数字×d；                如果当前位为 1，那么高位可选的可能是 (0、1、2、high-1) 一共 high 种，低位随意组合，一共 digit 种
如果d位的数字是1，则1出现的次数为左边数字×d + 右边数字 + 1；  如果当前位为 1，那么(高位可选的可能是 (0、1、2、high-1) 一共 high 种&&低位随意组合) +(高位是 hight 不变一种可能，低位包含 000..000 一共 low + 1 种)
如果d位的数字大于1，则1出现的次数为(左边数字+1)×d。          如果当前位是 1，那么高位可选 0,1,2...hight，一共 hight + 1 种可能，低位随意组合，一共 digit 种


class Solution {
public:
    int countDigitOne(int n) {
        long long int digit = 1;  // digit表示位的级别，初始从个位开始
        int count = 0;  // 记录出现1的次数

        int high = n / 10, cur = n % 10, low = 0; 
        // high是当前位的高位数字；cur是当前位上的数字；low是当前位的低位数字
